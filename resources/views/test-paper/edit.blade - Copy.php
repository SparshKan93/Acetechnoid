            ...
            <div id="que_paper" class="box wow fadeInUp tooltipcss" style="min-height: 30em">
              ...
              <ol id="body">
                   @foreach($data as $key => $value)
                    <li class="que_type">
                        <span class="editable" data-id="{{$value->que_type}}">{!!$value->name!!}</span>
                        <span class="editable marking">{{count($value->ques)}}x1</span>
                        <ol type="a" id="{{$value->que_type}}">
                            @foreach($value->ques as $key2 => $list)
                            @php $val = str_replace(['&nbsp;</span></p>', '</span></div><div>', '</div></div><div><br></div>'], '', $list->question)  @endphp
                            <li class="editable" data-id="{{$list->id}}">{!!$val!!}</li>
                            @endforeach
                        </ol>
                    </li>
                    @endforeach
