...
<!--==========================
    ...
    <section id="pricing" class="section-bg" style="padding: 100px 0 60px 0;">
      <div class="container">
        ...
        <div class="row">
          ...
          <div class=" col-lg-8 col-md-8 flex">
            <div id="que_paper" class="box wow fadeInUp tooltipcss" style="min-height: 30em">
              ...
              <h3 class="">Please Fill The Details</h3>
              <p class="editable marking">Required/All Questions x Marks<input type="hidden" name="marks" value="100"></p><br>
               <ol id="body">
                  @php $count = 0 @endphp
                   @foreach($data as $key => $value)
                    <li class="que_type " style="margin-bottom: 10px;">
                        <span class="" data-id="{{$value->que_type}}">{!!$value->name!!}</span>
                        <span class="marking">
                          <input class="required-que" type="text" name="question[{{$value->que_type}}][questions][]" value="{{sprintf('%02d', count($value->ques))}}" style="width:35px;height:25px;">
                          /<span class="avilable-questions">{{sprintf("%02d", count($value->ques))}}</span> x 
                          <input class="marks" type="text" name="question[{{$value->que_type}}][marks][]" value="1" style="width:35px;height:25px;"> =
                          <div class="total" style="display:inline;"> 
                              {{sprintf('%02d', count($value->ques))}}
                          </div>

                          @php $count += count($value->ques) @endphp
                        </span>
               ...
               <!-- <div>{{$count}}</sdiv> -->
               <div>Total Questions Selected <span id="tol_que_req"></span></div>
               <div>Maxium Marks <span id="total_marks"></span></div>
        ...
        @foreach($chapters_selected as $key => $list)
          <input type="hidden" name="chapters[]" value="{{$list}}">
        @endforeach
...
<script type="text/javascript">
  $('.required-que, .marks').keyup(function(){

    var que_req = $(this).closest('.marking').find('.required-que').val() != ''? $(this).closest('.marking').find('.required-que').val(): 0;
    var marks = $(this).closest('.marking').find('.marks').val() != ''? $(this).closest('.marking').find('.marks').val(): 0;
    var total = $(this).closest('.marking').find('.total').val() != ''? $(this).closest('.marking').find('.marks').val(): 0;
    var que_avi = $(this).closest('.marking').find('.avilable-questions').text() != ''? $(this).closest('.marking').find('.avilable-questions').text(): 0;

    if(parseInt(que_req) <= parseInt(que_avi))
      $(this).closest('.marking').find('.total').text(que_req*marks);
    else 
      alert('Required Number of question cannot be greater than avilable question!');

    var tol_que_req = 0;
    var total_marks = 0;

    $("[class *= 'required-que").each(function(){ 
      tol_que_req += +$(this).val();
    });

    $("[class *= 'total").each(function(){ 
      total_marks += +$(this).text();
    });

    $('#tol_que_req').text(tol_que_req);
              ...
              $('#que_body li').remove();
                for (i=0;i<data.length;i++)
                {
                    ...
                    { 
                      ...
                      selOpts += "<li class='question_add' data-id='"+id+"' data-answer='"+ques[j].answer+"'>"+que.replace("./Uploads","/Uploads")+"</li>";
                    }
  ...
  $(document).ready(function(){
    ...
    $('#que_paper').on('dblclick', '.editable', function() {
      ...
      if($('#que_paper').find("textarea").val()){
        ...
        $(this).html("<textarea row='1'>"+text+"</textarea><span style='display: none;'>"+text+"</span>");
      }
    });

    $('#que_paper').on('focusout', '.editing textarea', function() {
      ...
      if($(this).closest(".editing").prop("tagName") == 'LI'){
        ...
        $(this).closest(".editable").html(text+span);
      }
      else{
        $(this).closest(".editable").toggleClass("editing");
        $(this).closest(".editable").html(text);
      }
