...
class TestPaper extends Model
{
    ...
    public static function que_type($chapter_num,$ebook_id)
    {
        ...
        foreach ($data as $key => $value) {
            $value->ques = PaperGenerator::select('id', 'question', 'que_type', 'answer')->where(['ebook_id' => $ebook_id, 'chapter_num' => $chapter_num, 'que_type' => $value->que_type])->get();
        }
    ...
    public static function queTypeAuto($chapter_num,$ebook_id)
    {
        ...
        foreach ($data as $key => $value) {
            $value->ques = PaperGenerator::select('id', 'question', 'que_type', 'answer')->where(['ebook_id' => $ebook_id, 'que_type' => $value->que_type])
                ->whereIn('chapter_num', $chapter_num)
                ->get();
        }
